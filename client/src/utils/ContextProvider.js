...
function calculateDesk(w, d, h){
	...
	//Calculating plywood material below
	let plywoodAreaInFeet = ((w * d)+(2*(12*d))+(12*w)+(8*(w-3))+117) / 144;
	let plywoodCount = plywoodRound(plywoodAreaInFeet);
	let plywoodWasteFactorPercentage =
		((plywoodCount * 32 - plywoodAreaInFeet) / (plywoodCount * 32)) * 100;
	console.log(
...
function calculateDresser(w, d, h){
	...
	//Calculating plywood material below
	let plywoodAreaInFeet = ((2*((h-3.5)*d))+((h-3.5)*w)+(2*(w*(d-0.75)))+(w*d)+(h-4.25)+(4*((2*(d-3))+((w-4)*(d-3))+((h-4.75)/4))))/144;
	let plywoodCount = plywoodRound(plywoodAreaInFeet);
	let plywoodWasteFactorPercentage =
		((plywoodCount * 32 - plywoodAreaInFeet) / (plywoodCount * 32)) * 100;
	console.log(
...
let cementBoardWasteFactorPercentage =
	((cementBoardCount * 32 - cementBoardAreaInFeet) / (cementBoardCount * 32)) * 100;
console.log(
	"You are using" +
		plywoodCount +
		" pieces of plywood at a waste factor of " +
...
function calculateWall(w, d, h){
	...
	let gypsumBoardWasteFactorPercentage =
		((gypsumBoardCount * 32 - gypsumBoardAreaInFeet) / (gypsumBoardCount * 32)) * 100;
	console.log(
		"You are using" +
		gypsumBoardCount +
